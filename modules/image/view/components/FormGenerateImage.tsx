...
import { GenerateImageDto, generateImageSchema } from "../../application/dtos/request/generate-image.dto";
...
export default function FormGenerateImage() {
  ...
  const isGenerating = useSelector((state: RootState) => state.image.isGenerating);
  ...
  const currentStyleConfig = Styles[selectedStyleId as TypeStyle] || Styles[TypeStyle.DIGITAL_ART];
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="space-y-1">
        ...
        />
        {errors.prompt && <p className="text-red-500 text-xs font-medium">{errors.prompt.message}</p>}
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pt-2">
        <div className="flex items-center gap-2">
          {[
            ...
            { id: "landscape", ratio: "16:9", w: 1792, h: 1024, label: "Landscape" },
            { id: "portrait", ratio: "9:16", w: 1024, h: 1792, label: "Portrait" },
          ].map((item) => (
            <label
              key={item.id}
              ...
              <input
                ...
                {...register("aspectRatio", {
                  onChange: (e) => {
                    ...
                    const config = { "1:1": [1024, 1024], "16:9": [1792, 1024], "9:16": [1024, 1792] }[val as "1:1" | "16:9" | "9:16"];
